...
Review the scanned schema before accepting a warehouse/SQL source with `ds verify --accept-schema`. File uploads also require `ds verify`, without that flag - for them that call is the activation. Confirm the returned source is ready and verified before reporting activation; scan completion alone is not activation. `ds verify` also prints every column the PII detector hid - masked as NULL in every query, dashboards and exports included - with its reason and the exact `kb column-visibility <model> <COLUMN> visible` line; surface that list to the user before or as you activate. For a false positive, `ds verify <id> --expose COL1,COL2` unhides the named columns as part of the same verify (the source's creator or an org admin; it works on an already-verified source too); the JSON reports `pii_hidden`, `can_expose_verdict`, `exposed` and `expose_failed`.
